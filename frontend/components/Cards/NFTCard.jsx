...
const NFTCard = ({ url, stake, tokenId, address }) => {
  console.log(address);
  ...
  const stakeNft = async () => {
    const txs = [];
    const approve = new ethers.utils.Interface(NFTAbi.abi);
    const staking = new ethers.utils.Interface(StakingAbi.abi);

    const data1 = approve.encodeFunctionData("setApprovalForAll", [
      StakingAbi.address,
      true,
    ]);
    const tx1 = {
    ...
    txs.push(tx1);
    const data2 = staking.encodeFunctionData("stakeNFT", [nft.tokenID]);
    const tx2 = {
    ...
    const txResponse = await address.sendGaslessTransactionBatch({
      transactions: txs,
    });
