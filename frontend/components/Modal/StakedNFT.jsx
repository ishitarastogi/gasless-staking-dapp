...
console.log("STAKED",smartAccount?.address);

  async function getStaked() {
    if (smartAccount?.address) {
      console.log("STAKED2",smartAccount?.address);
    ...
    if (smartAccount?.address) {
      const getReward = async () => {
        try {
          const reward = await stakingContract?.calculateReward(smartAccount?.address);
  ...
  return (
    <div className="flex flex-col mx-auto text-center">
        <button onClick={getStaked}>See your staked NFTs</button>      <div className="mx-auto my-7">
      
        {tokenURI && rewardBal ? (
          <NFTCard url={tokenURI} stake={false} tokenId={tokenId} address={smartAccount} />
        ) : (
